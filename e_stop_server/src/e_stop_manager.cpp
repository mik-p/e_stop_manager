#include <e_stop_server/e_stop_manager.h>
...
EStopManager::EStopManager(ros::NodeHandle& nh, ros::NodeHandle& pnh) : nh_(nh), pnh_(pnh)
{
  ...
  {
    ROS_ERROR_NAMED("e_stop_manager", "Parameter e_stop_topic not provided or is not a list! E-stops will only "
                                      "published on e-stop-list topic!");
  }
  else
  {
    ...
    {
      // here there is only one entry per list entry (e.g. not as in e_stop_list where there are two entries (name and
      // value) per list entry)
  ...
  for (auto& pub : e_stop_pub_)
  {
    pub.second.erase(std::remove_if(pub.second.begin(), pub.second.end(),
                                    [&](const std::string& name) {
                                      ...
                                      if (std::find(e_stop_list_msg_.names.begin(), e_stop_list_msg_.names.end(),
                                                    name) == e_stop_list_msg_.names.end())
                                      {
                                        ROS_WARN_STREAM_NAMED("e_stop_manager", "The e_stop named \"" << name
                                                                                                      << "\" was in "
                                                                                                         "e_stop_"
                                                                                                         "topics but "
                                                                                                         "not in "
  ...
  // init plugins
  XmlRpc::XmlRpcValue e_stop_propagators;

  // check if parameter e_stop_propagators exists and is a list
  if (pnh_.getParam("e_stop_propagators", e_stop_propagators))
  {
    ROS_ASSERT(e_stop_propagators.getType() == XmlRpc::XmlRpcValue::TypeArray);

    ROS_INFO_NAMED("e_stop_manager", "loading parameter e_stop_propagators");

    // load propagators via pluginlib
    // throw exception on init failure
    std::string package("e_stop_propagators");
    std::string p_base("e_stop_manager::EStopPropagator");
    // store libs in class members
    p_loader_ = std::make_shared<pluginlib::ClassLoader<e_stop_manager::EStopPropagator>>(package, p_base);

    // load propagators
    for (int32_t i = 0; i < e_stop_propagators.size(); ++i)
    {
      XmlRpc::XmlRpcValue::iterator iter;
      std::string propagator_name;
      std::string propagator_type;

      for (iter = e_stop_propagators[i].begin(); iter != e_stop_propagators[i].end(); iter++)
      {
        ROS_ASSERT(iter->second.getType() == XmlRpc::XmlRpcValue::TypeString);

        // get propagator name and type
        if (iter->first == "name")
        {
          propagator_name = (std::string)iter->second;
        }
        if (iter->first == "type")
        {
          propagator_type = (std::string)iter->second;
        }
      }
      ...
      try
      {
        boost::shared_ptr<EStopPropagator> propagator = p_loader_->createInstance(propagator_type);
        propagator->init(std::make_shared<ros::NodeHandle>(nh_),
                         std::make_shared<ros::NodeHandle>(ros::this_node::getName() + "/" + propagator_name));
        propagator_list_.push_back(propagator);
        ROS_INFO_STREAM("loaded propagator: " << propagator_type);
      }
      catch (pluginlib::PluginlibException& ex)
      {
        ROS_ERROR("e stop propagator plugin %s failed to load: %s", propagator_type.c_str(), ex.what());
      }
    }
  }
...
void EStopManager::publishEStops(bool force_e_stop)
{
  ...
  {
    std_msgs::Bool msg;
    msg.data = force_e_stop || std::any_of(pub.second.begin(), pub.second.end(), [&](const std::string& name) {
    ...
    {
      prop_ptr->propagate(pub.first.getTopic(), msg);
    }
  }
}
}  // namespace e_stop_manager
